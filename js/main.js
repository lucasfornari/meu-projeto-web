...
if (listaDepoimentos) {
  carregarDepoimentos().then((dados) => {
    listaDepoimentos.innerHTML = dados
      .map(
        (d) =>
          `<div class="col-md-4">
...
if (checkboxes.length > 0) {
  function calcularTotal() {
    ...
    checkboxes.forEach((cb, i) => {
      if (cb.checked)
        total += parseFloat(cb.value) * parseInt(quantidades[i].value);
    });
  ...
  document.querySelectorAll('.btn-adicionar').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (!checkboxes[idx].checked) {
        alerta(
          'alert-carrinho',
          'Selecione o produto antes de adicionar.',
          'warning'
        );
        return;
      }
      ...
      if (item) item.quantidade += parseInt(quantidades[idx].value);
        ...
        carrinho.push({
          nome,
          preco: parseFloat(checkboxes[idx].value),
          quantidade: parseInt(quantidades[idx].value),
        });
  ...
  document
    .getElementById('btn-fechar-modal-footer')
    ?.addEventListener('click', fecharModal);
...
document.getElementById('cep')?.addEventListener('blur', async function () {
  ...
  if (dados.erro) {
    alerta('alert-form', 'CEP não encontrado.', 'danger');
    return;
  }
...
document
  ...
  ?.addEventListener('submit', async function (e) {
    ...
    if (res.status === 201) {
      alerta('alert-form', 'Mensagem enviada!', 'success');
      this.reset();
...
function renderizarCarrinho() {
  ...
  var total = 0;
  lista.innerHTML = `<ul class="cart-list">${carrinho
    .map((p, i) => {
...
if (document.getElementById('carrinhoLista')) {
  ...
  document
    ...
    ?.addEventListener('submit', function (e) {
      ...
      var carrinho = JSON.parse(localStorage.getItem('carrinho') || '[]');
      if (!carrinho.length) {
        alerta('alert-compra', 'Carrinho vazio!', 'warning');
        return;
      }
      var metodo = document.querySelector(
